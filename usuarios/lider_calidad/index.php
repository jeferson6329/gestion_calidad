...
<div class="alert alert-info text-center" role="alert">
    ...
    <span class="badge badge-warning"><?php echo $totales_estado['en revision']; ?></span>
    <br>
    ...
    <div class="row justify-content-center">
        ...
        <div class="col-auto">
            <a href="monitoreos.php?estado=en revision" class="btn btn-warning">
                En revision <span class="badge badge-light"><?php echo $totales_estado['en revision']; ?></span>
            </a>
    </div>
</div>
...
<!-- Totales por líder de calidad -->
<div class="container mb-3">
    <div class="card">
        <div class="card-header text-center">
            <strong>Monitoreos realizados por cada líder de calidad</strong>
        </div>
        <div class="card-body">
            <table class="table table-bordered table-sm">
                <thead>
                ...
                <tbody>
                    <?php
                    // Monitoreos este mes y total por líder de calidad (rol flexible)
                    $consulta_lideres = "
                        SELECT p.id, p.nombre,
                            SUM(CASE WHEN MONTH(m.fecha_monitoreo) = ? AND YEAR(m.fecha_monitoreo) = ? THEN 1 ELSE 0 END) AS total_mes,
                            COUNT(m.id) AS total_general
                        FROM personas p
                        LEFT JOIN monitoreos m ON m.id_usuario = p.id
                        WHERE LOWER(REPLACE(p.rol, '_', ' ')) = 'lider de calidad'
                        GROUP BY p.id, p.nombre
                    ";
                    $stmt_lideres = $conexion->prepare($consulta_lideres);
                    $stmt_lideres->execute([$mes_actual, $anio_actual]);
                    ...
                    ?>
                        <tr>
                            ...
                            <td><?php echo $lider['total_mes']; ?></td>
                            <td><?php echo $lider['total_general']; ?></td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
...
<!-- Reporte mensual horizontal por agente (últimos 6 meses) -->
    ...
    <div class="card">
        ...
        <div class="card-body">
            <table class="table table-bordered table-sm text-center">
                <thead>
                    <tr>
                        ...
                        for ($i = 5; $i >= 0; $i--) {
                            ...
                            $labels[] = ucfirst(strftime('%B', $fecha)) . " $anio_num";
                        }
                ...
                <tbody>
                    <?php
                    // Consulta para obtener promedios por agente y mes (solo últimos 6 meses)
                    $fechas_where = [];
                    foreach ($meses as $m) {
